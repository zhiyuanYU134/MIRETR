...
from dataset import Scan2cad_test_data_loader
...
def run_one_epoch(
    ...
    for i, data_dict in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            #corr_tensor,index,couts=torch.unique(corr_tensor, return_inverse=True,return_counts=True,dim=0)
            align_src_points = apply_transform(corr_tensor[:,:3].unsqueeze(0), estimated_transforms_gt)
            rmse = torch.linalg.norm(align_src_points - corr_tensor[:,3:].unsqueeze(0), dim=-1)<(0.05)
...
def main():
    ...
    with Engine(log_file=log_file, default_parser=parser, seed=config.seed) as engine:
        engine.args.test_epoch=44

        start_time = time.time()

        test_loader = Scan2cad_test_data_loader(engine, config)
